...
export const SUMMARY_GRID_CELL_ATTR = "data-summary-grid-cell";
...
export function parseSummaryGridCellElement(el: Element | null): GridCell | null {
  if (!el) return null;
  const cell = el.closest(`[${SUMMARY_GRID_CELL_ATTR}]`);
  if (!cell) return null;
  const row = Number(cell.getAttribute("data-summary-row"));
  const col = Number(cell.getAttribute("data-summary-col"));
  if (!Number.isFinite(row) || !Number.isFinite(col)) return null;
  return { row, col };
}
...
export function findSummaryGridCellFromPoint(x: number, y: number): GridCell | null {
  const el = document.elementFromPoint(x, y);
  return parseSummaryGridCellElement(el);
}
