...
Infrastructure as code and GCP operations documentation for LogiVoice. This page is the **canonical runbook** for Terraform: login, stack order, remote backends, and `-var-file` usage. For app module details (variables, Cloud Run, Cloud SQL, images, schedules), see [terraform/README.md](terraform/README.md).
...
| Topic | Path |
|-------|------|
| Terraform app stack (Cloud Run api/web/worker, GCS, Cloud SQL MySQL, Memorystore Redis, Pub/Sub; optional SQL weekend schedule) | [terraform/README.md](terraform/README.md) |
| Cloud Build + deploy | [cloudbuild/README.md](../../cloudbuild/README.md) |
...
After infrastructure exists, build and deploy images with [Cloud Build](../../cloudbuild/) (see [cloudbuild/README.md](../../cloudbuild/README.md); worker: `cloudbuild.dev.worker.yaml`).
...
terraform init -reconfigure -backend-config=backend.tf.dev -upgrade
...
**Purpose:** Cloud Run, optional GCS, Cloud SQL, Memorystore, Pub/Sub, optional SQL weekend schedule, etc., for one environment per configuration.
...
- If **`enable_memorystore_redis = true`** (or **`enable_cloud_sql`**), apply **`network/`** first and set **`network_remote_state_*`** in app tfvars.

### Firebase Authentication (outside Terraform defaults)

Firebase is linked to each GCP environment project in **Firebase Console** (Authentication, authorized domains). Do **not** commit API keys:

- **`NEXT_PUBLIC_FIREBASE_*`**: bake at **web Docker build time** via Cloud Build substitutions (`_NEXT_PUBLIC_*`), aligned with the Firebase Web app settings.
- **Server `FIREBASE_*`**: store in **Secret Manager** and attach via **`api_secret_env_from_sm`** / Cloud Run `--set-secrets`.

Optional later: enable Identity Platform / Firebase resources via IaC (`google_project_service`, etc.) if your org requires it.
...
| Cloud Build | [cloudbuild/README.md](../../cloudbuild/README.md) || Manual Cloud Run | *(add your runbook link if applicable)* |
