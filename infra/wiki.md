...
| `service`    | Tên sản phẩm / dịch vụ (chữ thường, cho phép dấu gạch ngang)             | `logivoice` |
| `env`        | Môi trường (`dev` / `stg` / `prod` / `common`)                           | `prod` |


**Ví dụ:** `logivoice-dev-dev`, `logivoice-dev-prod`, `logivoice-dev-common`
...
| `service` | `logivoice`                                | Tên sản phẩm / dịch vụ                           |
...
gcloud projects list --filter="labels.service=logivoice"
```
...
| -------------------- | -------------------------- | ----------------------- |
| Cloud Run            | `{service}-{component}`    | `logivoice-api`              |
| Cloud SQL            | `{service}-{engine}-{env}` | `logivoice-pg-prod`          |
| Cloud Build trigger  | `{service}-{branch}-build` | `logivoice-main-build`       |
| Artifact Registry    | `{service}-docker`         | `logivoice-docker`           |
| Cloud Storage bucket | `{project-id}-{dùng cho}`  | `logivoice-dev-prod-assets` |
| VPC                  | `{project-id}-vpc`         | `logivoice-dev-prod-vpc`    |
...
| `service`    | プロダクト・サービス名（小文字英数字、ハイフン可）                | `logivoice` |
| `env`        | 環境識別子（`dev` / `stg` / `prod` / `common`） | `prod` |


**例:** `logivoice-dev-dev`、`logivoice-dev-prod`、`logivoice-dev-common`
...
| Cloud Run          | `{service}-{component}`    | `logivoice-api`              |
| Cloud SQL          | `{service}-{engine}-{env}` | `logivoice-pg-prod`          |
| Cloud Build トリガー   | `{service}-{branch}-build` | `logivoice-main-build`       |
| Artifact Registry  | `{service}-docker`         | `logivoice-docker`           |
| Cloud Storage バケット | `{project-id}-{用途}`        | `logivoice-dev-prod-assets` |
| VPC                | `{project-id}-vpc`         | `logivoice-dev-prod-vpc`    |
