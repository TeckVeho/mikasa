...
locals {
  ...
  # Same sizing as wiki web row until Git ref includes module.tier_specs.worker (mirror in terraform-modules).
  tier_worker = {
  ...
  cloud_run_worker_min_instances_effective = coalesce(var.cloud_run_worker_min_instances, local.tier_worker.min_instances)
  cloud_run_worker_max_instances_effective = coalesce(var.cloud_run_worker_max_instances, local.tier_worker.max_instances)
  cloud_run_worker_cpu_effective           = coalesce(var.cloud_run_worker_cpu, local.tier_worker.cpu)
  cloud_run_worker_memory_effective        = coalesce(var.cloud_run_worker_memory, local.tier_worker.memory)
  cloud_run_worker_timeout_effective       = coalesce(var.cloud_run_worker_timeout, local.tier_worker.timeout)
  cloud_run_worker_concurrency_effective   = coalesce(var.cloud_run_worker_concurrency, local.tier_worker.concurrency)
