...
locals {
  ...
  sql_schedule_fn_name = "logivoice-sql-schedule-${var.env_suffix}"
}
...
resource "google_service_account" "sql_schedule_fn" {
  ...
  account_id   = "logivoice-sql-schedule-${var.env_suffix}"
  display_name = "LogiVoice Cloud SQL schedule (Functions Gen2)"
  project      = var.project_id
}
...
resource "google_service_account" "scheduler_invoker" {
  ...
  account_id   = "logivoice-sched-sql-${var.env_suffix}"
  display_name = "LogiVoice Cloud Scheduler → SQL schedule function (OIDC)"
  project      = var.project_id
}
...
resource "google_storage_bucket" "cf_source" {
  ...
  name                        = substr(replace(lower("${var.project_id}-logivoice-cf-src-${var.env_suffix}"), "_", "-"), 0, 63)
  location                    = var.region
...
resource "google_cloud_scheduler_job" "sql_stop" {
  ...
  name        = "logivoice-sql-stop-${var.env_suffix}"
...
resource "google_cloud_scheduler_job" "sql_start" {
  ...
  name        = "logivoice-sql-start-${var.env_suffix}"
