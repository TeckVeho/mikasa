...
locals {
  ...
  api_container_env = merge(
    var.enable_gcs ? {
      GCS_BUCKET_NAME = local.gcs_bucket_name_effective
      GCP_PROJECT_ID  = var.project_id
    } : {},
    var.enable_pubsub_call_pipeline ? {
      GOOGLE_CLOUD_PROJECT        = var.project_id
      PUBSUB_TOPIC_CALL_COMPLETED = google_pubsub_topic.call_completed[0].name
    } : {},
...
resource "google_artifact_registry_repository" "docker" {
  ...
  description   = "LogiVoice Docker images (Terraform)"
  format        = "DOCKER"
...
check "web_image_when_enabled" {
  assert {
    ...
    error_message = "When enable_web is true, set web_container_image to a pushed logivoice-web image (e.g. .../logivoice-web:dev)."
  }
}
...
check "network_remote_state_when_vpc_required" {
  assert {
    condition     = !local.network_stack_required || (var.network_remote_state_bucket != "" && var.network_remote_state_prefix != "")
    error_message = "When Cloud SQL or Memorystore Redis is enabled, set network_remote_state_bucket and network_remote_state_prefix. Apply infra/terraform/network first."
  }
}
