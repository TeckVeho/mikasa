...
output "cloud_sql_instance_name" {
  description = "Cloud SQL instance id used by this stack (wiki default: logivoice-mysql-{env_suffix})."
...
output "api_public_base_url" {
  ...
  value       = local.api_custom_domain_fqdn != "" ? "https://${local.api_custom_domain_fqdn}" : trimsuffix(google_cloud_run_v2_service.api.uri, "/")
}
...
output "web_public_base_url" {
  ...
  value       = !var.enable_web ? null : (local.web_custom_domain_fqdn != "" ? "https://${local.web_custom_domain_fqdn}" : trimsuffix(google_cloud_run_v2_service.web[0].uri, "/"))
}
...
output "cloud_run_migrate_job_name" {
  description = "Cloud Run Job name for npx prisma migrate deploy (null if enable_cloud_sql is false)."
...
output "memorystore_redis_host" {
  description = "Memorystore Redis primary endpoint (null if enable_memorystore_redis is false)."
  value       = var.enable_memorystore_redis ? google_redis_instance.main[0].host : null
}

output "worker_cloud_run_uri" {
  description = "Worker Cloud Run URL (null if enable_worker is false)."
  value       = var.enable_worker ? google_cloud_run_v2_service.worker[0].uri : null
}

output "pubsub_call_completed_topic" {
  description = "Pub/Sub topic name for call-completed pipeline (null if disabled)."
  value       = var.enable_pubsub_call_pipeline ? google_pubsub_topic.call_completed[0].name : null
}

output "pubsub_summarize_subscription" {
  description = "Pub/Sub summarize-worker subscription resource name."
  value       = var.enable_pubsub_call_pipeline ? google_pubsub_subscription.summarize_worker[0].name : null
}
