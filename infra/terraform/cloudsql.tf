...
locals {
  # Wiki: {service}-mysql-{env} → logivoice-mysql-dev. Set var.sql_instance_name to keep an older instance id.
  sql_instance_name_effective = substr(
    replace(
      lower(var.sql_instance_name != "" ? var.sql_instance_name : "logivoice-mysql-${var.env_suffix}"),
      "_",
      "-",
    ),
    0,
...
resource "google_secret_manager_secret" "database_url" {
  ...
  secret_id = "logivoice-database-url-${var.env_suffix}"
