...
Single GCP project (`mikasa-lm-dev`); environments separated by `env_suffix` in tfvars.
...
- Remote state bucket: `mikasa-lm-dev-terraform-state` (in `live/*/terragrunt.hcl`).
- Keep `project_id` in `terraform.tfvars` aligned with `GCP_PROJECT_ID` in CI (`mikasa-lm-dev`).
