...
variable "enable_worker" {
  type        = bool
  default     = true
  description = "Create Pub/Sub + Cloud Run worker (summarize pipeline). Set false for API/Web-only stacks."
}
...
variable "enable_memorystore" {
  ...
  default     = true
  description = <<-EOT
    If true (and enable_cloud_sql = true), provision Cloud Memorystore Redis and inject REDIS_* env on API/worker.
    Evry bulletin board does not require Redis — set false on dev to avoid Memorystore cost.
  EOT
}
