...
Always run `gcloud builds submit` from **`LogiVoice/`** (monorepo root with `package-lock.json`, `apps/`, `packages/`).
...
|----------------|-----------|
| `packages/shared/**`, root `package-lock.json`/`turbo`/`tsconfig.base.json`, `cloudbuild/**`, `.github/workflows/**`, `apps/*/Dockerfile` | Treated as **full stack** → one Cloud Build (`cloudbuild.dev.yaml` / `cloudbuild.prod.yaml`) for API (with migrate) + web + worker — single build VM. |
